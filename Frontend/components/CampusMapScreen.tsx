...
const COORD_MAP: Record<string, { lat: number, lng: number }> = {
    ...
    "Student Rec": { lat: 30.6094, lng: -96.3400 },
    "Southside": { lat: 30.6105, lng: -96.3364 },
    ...
    "MSL": { lat: 30.6120, lng: -96.3533 }, // Medical
    "Medical": { lat: 30.6120, lng: -96.3533 },
    "PSEL": { lat: 30.6151, lng: -96.3510 },
    
    // Other known explicit buildings from events
    "Memorial Student Center": { lat: 30.6123, lng: -96.3415 },
    "MSC": { lat: 30.6123, lng: -96.3415 },
    "Rudder": { lat: 30.6130, lng: -96.3406 },
    "Wisenbaker": { lat: 30.6202, lng: -96.3400 },
...
const MAIN_REC_KEYWORDS = ["Court", "Field", "Locker", "Boulder Wall", "Climbing", "Pool", "Spa", "Strength", "Dive", "50-Meter", "Turf", "Gym", "Rec", "Instructional", "Racquetball", "Squash", "Volleyball", "Bag Room", "Sauna"];
const IGNORED = ["Zoom", "Virtual", "Online", "Unknown Event Location", "Multiple dates", "TX, USA", "Online"];
const MAIN_REC_COORD = { lat: 30.6094, lng: -96.3400 };
    ...
    const fetchTraffic = async () => {
        try {
            ...
            const mapped = data.map((d: any) => {
                ...
                // 1. Exact string fuzzy matching (jitter coordinates lightly so layers do not overlap fully)
                for (const [key, val] of Object.entries(COORD_MAP)) {
                    if (safeLocation.toLowerCase().includes(key.toLowerCase())) {
                        coord = { lat: val.lat + (Math.random()-0.5)*0.0005, lng: val.lng + (Math.random()-0.5)*0.0005 };
                        break;
                    }
                }
                
                // 2. Main Rec internal layout matching (Catch-all for 'Court', 'Pool', etc)
                if (!coord && MAIN_REC_KEYWORDS.some(k => safeLocation.toLowerCase().includes(k.toLowerCase()))) {
                    coord = { lat: MAIN_REC_COORD.lat + (Math.random()-0.5)*0.001, lng: MAIN_REC_COORD.lng + (Math.random()-0.5)*0.001 };
                }

                // 3. Fallback for random valid campus events (exclude generic string)
                if (!coord && !IGNORED.some(k => safeLocation.toLowerCase().includes(k.toLowerCase())) && safeLocation.trim().length > 3) {
                    coord = {
                        lat: 30.6153 + (Math.random() - 0.5) * 0.02,
                        lng: -96.341 + (Math.random() - 0.5) * 0.02,
                    };
                }
    ...
    if (loading) {
        return (
            <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
                ...
                <Text style={{ marginTop: 12, color: COLORS.textSecondary }}>Loading live campus traffic...</Text>
            </View>
        );
    }
                ...
                {locations.map((loc, idx) => {
                    ...
                    return (
                        <React.Fragment key={`${loc.location}-${idx}`}>
                            ...
                            >
                                <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: stroke }} />
                            </Marker>
                        </React.Fragment>
                    );
                })}
