...
# Load REDIS_URL / UPSTASH_REDIS_URL from .env
...
if env_path.exists():
    with open(env_path, "r") as f:
        for line in f:
            if line.startswith("REDIS_URL=") or line.startswith("UPSTASH_REDIS_URL="):
                url = line.strip().split("=", 1)[1].strip()
                if url:
                    break

if not url:
    print(f"REDIS_URL / UPSTASH_REDIS_URL not found in .env at {env_path}")
    exit(1)

print(f"Connecting to Redis cache: {url[:30]}...")
