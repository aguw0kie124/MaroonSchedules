...
class AggieSpiritProxy:
    ...
    def get_pattern(self, route_key: str) -> Dict[str, Any]:
        try:
            ...
            # Fetch base data to resolve directionKey -> directionName
            base_data = self._post("/RouteMap/GetBaseData/", "")
            direction_map = {}
            if isinstance(base_data, dict) and "routes" in base_data:
                for route in base_data["routes"]:
                    for dir_item in route.get("directionList", []):
                        d_key = dir_item.get("direction", {}).get(
                            "key") or dir_item.get("key")
                        d_name = dir_item.get("direction", {}).get(
                            "name") or dir_item.get("name")
                        if d_key and d_name:
                            direction_map[d_key] = d_name
            ...
            if data:
                for item in data[0].get("patternPaths", []):
                    dir_obj = item.get("direction") or {}
                    dir_key = dir_obj.get("key") or item.get(
                        "directionKey") or ""

                    # Look up the actual direction name using the map we built
                    dir_name = dir_obj.get("name") or item.get(
                        "directionName") or direction_map.get(dir_key, "")
            ...
            print(f"Error in get_pattern: {e}")
